...
- **Standardised mean differences (SMD)**: Optional SMD column for balance assessment
- **Summary types**: 
  - Median (IQR)
  - Mean (SD)
...
Use `include_smd = TRUE` to add an SMD column. SMD is only valid for 2-level strata — a warning will be issued and the column skipped if strata has more levels.
```r
# Two-level strata required for SMD
mtcars$am <- factor(mtcars$am, labels = c("Automatic", "Manual"))

table_smd <- make_output_df(mtcars, "am", include_smd = TRUE, round = 2) %>%
  get_median_iqr(variable = 'mpg', name = "Miles per gallon") %>%
  get_mean_sd(variable = 'hp', name = "Horsepower") %>%
  get_n_percent(variable = 'gear', name = "Number of gears") %>%
  extract_df()

knitr::kable(table_smd)
```
SMD and statistical tests can be requested together:
```r
table_both <- make_output_df(mtcars, "am", include_tests = TRUE, include_smd = TRUE, round = 2) %>%
  get_median_iqr(variable = 'mpg', name = "Miles per gallon") %>%
  get_n_percent(variable = 'gear', name = "Number of gears") %>%
  extract_df()
```
