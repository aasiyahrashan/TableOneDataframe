...
- Standardised mean differences (SMD) for balance assessment
...
Use `include_smd = TRUE` to add an SMD column for balance assessment. SMD is computed using the `smd` package and is only meaningful for 2-level strata — a warning will be issued and the column skipped if strata has more levels.

SMD is reported on the header row for categorical variables, and on the data row for continuous variables.
```{r smd}
# SMD requires a 2-level strata variable
...
If you request SMD with 3+ strata levels, it is silently skipped with a warning:
```{r smd-warning}
# cyl has 3 levels — SMD will be skipped with a warning
table_no_smd <- make_output_df(mtcars, "cyl", include_smd = TRUE, round = 2) %>%
  get_median_iqr(variable = 'mpg', name = "Miles per gallon") %>%
  extract_df()
```
...
# Build comprehensive table with tests and SMD
complete_table <- make_output_df(mtcars, "am", include_tests = TRUE, 
                                 include_smd = TRUE, round = 1) %>%
  get_median_iqr(variable = 'mpg', name = "Miles per gallon") %>%
  get_mean_sd(variable = 'hp', name = "Horsepower") %>%
  get_median_iqr(variable = 'wt', name = "Weight (1000 lbs)") %>%
  get_n_percent(variable = 'cyl', name = "Cylinders") %>%
  get_n_percent(variable = 'gear', name = "Number of gears") %>%
  extract_df()
...
# Export to CSV
# write.csv(complete_table, "table1.csv", row.names = FALSE)

# Or use with other packages
# library(writexl)
# write_xlsx(complete_table, "table1.xlsx")
```
...
4. **Use `include_smd = TRUE`** for balance assessment — requires a 2-level strata variable
5. **Use meaningful row names** - they appear in the first column of your table
6. **The output is a standard dataframe** - use it with `knitr::kable()`, `writexl`, `write.csv()`, etc.
